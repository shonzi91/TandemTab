...
public static class AccountSnapshotSerializer
{
    ...
    private static Period ToEntity(PeriodNode n, string currency)
    {
        ...
        SetField(p, "_contributions", n.Contributions.Where(c => c.MemberId != Period.CarryoverSource).Select(c =>
        {
            ...
            contribution.SetClientId(c.ClientId);
            contribution.SetReconciliationFor(c.ReconciliationForPeriodId);
            return contribution;
        }).ToList());
        ...
        SetField(p, "_expenses", n.Expenses.Select(e =>
        {
            ...
            expense.SetReconciliationFor(e.ReconciliationForPeriodId);
            return expense;
        }).ToList());
        ...
        SetField(p, "_fundTransfers", n.FundTransfers.Select(t =>
        {
            ...
            transfer.SetClientId(t.ClientId);
            return transfer;
        }).ToList());
    ...
    private record ContributionNode(Guid Id, Guid MemberId, decimal Paid,
        ...
        // T0: the write's idempotency key. Trailing and optional — see ExpenseNode.ClientId.
        Guid? ClientId = null,
        // The rollover this deposit reconciles, when it is a reconciliation adjustment rather than real income.
        // Null on every ordinary deposit. See Contribution.ReconciliationForPeriodId.
        Guid? ReconciliationForPeriodId = null);
    ...
    private record ExpenseNode(Guid Id, Guid CategoryId, decimal Amount, DateOnly Date, Guid MemberId, Guid FundId, string? Note, Guid? SourceSavingCategoryId, bool OnBehalfOfOtherAccount = false,
        ...
        // T0: the idempotency key of the write that created this row, so a retry after an ambiguous failure finds
        // the expense instead of making a second one. Null on every node written before this existed and on every
        ...
        // The rollover this row reconciles, when it is a reconciliation adjustment. Null on every ordinary expense
        // and on every node written before the stamp existed — which reads as "not an adjustment we can undo", the
        // honest answer for rows nothing recorded an origin for. See Expense.ReconciliationForPeriodId.
        Guid? ReconciliationForPeriodId = null);
    ...
    private record FundTransferNode(Guid Id, Guid FromFundId, Guid ToFundId, decimal Amount, DateOnly Date, string? Note, bool FromSynced = false, bool ToSynced = false, string? BankExternalId = null, bool AutoFiled = false,
        // T0: the write's idempotency key. Distinct from BankExternalId above — that is the bank's key for a row it
        // filed, this is the client's key for a row a person asked for.
        Guid? ClientId = null);
    private record ExternalTransferNode(Guid Id, Guid FundId, decimal Amount, DateOnly Date, Guid? ToAccountId, string? Note, bool FundSynced = false,
        ...
        // T0: the write's idempotency key, on the outflow half only — see ExternalTransfer.ClientId for why one
        // side carries it for both.
        Guid? ClientId = null);
}
