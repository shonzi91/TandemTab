...
public record AddExpenseRequest(Guid CategoryId, decimal Amount, Guid FundId, DateOnly Date, string? Note = null, bool OnBehalfOfOtherAccount = false, Guid? TagId = null,
    ...
    TimeOnly? Time = null,
    // ★ T0 — the idempotency key. Generated by the client ONCE, when the draft is composed, and reused by every
    // attempt at sending it: a request that lands but whose response is lost is indistinguishable, from the
    // client's side, from one that never arrived, so the only useful thing it can do is send again. With a key the
