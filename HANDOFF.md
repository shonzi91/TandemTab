...
Last updated: 2026-07-31 (Session 73 — **big cross-cutting session: bank-callback native deep-link + Android bank-sync client, single-tag-per-expense across the whole stack, web Spending reworks (Goals-style header), external-login profile trims, Android profile parity (2FA/avatar/email); 3 web deploys, latest `finapp-00258-9wp`; committed to `main`**). Read this + [README.md](README.md) + [TRANSFER.md](TRANSFER.md) + [docs/MOBILE.md](docs/MOBILE.md) + [docs/DOMAIN-REMOVAL.md](docs/DOMAIN-REMOVAL.md) + [android/README.md](android/README.md) + recent `git log` to catch up.
