...
✅ **The key now exists (S110).** Generated by the owner — RSA 4096, PKCS12, 10,000 days, alias `tandemtab`, held
outside the repo and backed up; the four secrets are set. A `workflow_dispatch` run proved it: **Report the signer**
printed **the owner's DN** instead of `CN=Android Debug`, so the debug-key fallback is no longer in play and CI
artifacts are updatable in place. (The DN itself is not written down here — it is on every APK's certificate,
where it belongs, and this repo is public.)
⚠️ **Still true:** nothing is published, the AAB is not built, and there is no Play Console. **Distribution itself
moved to R7 by the owner** — the key was done early because it is a lead-time item, not because the door is opening.
...
1. ⛔ **The APK pipeline is now the binding constraint, and it outranks the parity count.** Two rows shipped this
   ...
   ✅ **The keystore was generated the same day (S110)** — it was a lead-time item, not a distribution item: an app
   installed with key A cannot be updated by an APK signed with key B, so every debug-signed build handed out
   before the key existed would have had to be uninstalled. CI now signs with the real key, verified by the signer
   DN on a `workflow_dispatch` run. **Nothing about distribution changed** — that is still R7's, by decision.
