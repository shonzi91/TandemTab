...
Last updated: 2026-07-11 (Session 23). Read this + [README.md](README.md) + [TRANSFER.md](TRANSFER.md) + recent `git log` to catch up.
...
**⏭ NEXT — money-out → transfer in bank review (requested, not yet built).** Today a bank **debit** in review can only be confirmed as an **expense** (category picker → `ConfirmBankTransaction`). Add the option to route it as a **transfer** to another fund — same account (fund→fund) or another account (`BudgetingState.TransferToAccount(destAcct, fromFund, amount, note, destFund)` already exists from the cross-account work). Row: `BankTxRow` (~L3202), debit branch has the category `<select>` bound to `_bankRowCat[id]` (Guid) → `ConfirmBankRow`→`ConfirmBankTransaction`. **Design caveat:** the debit path is entangled with the merchant→category auto-file/mapping machinery (`AutoHandleMappedDebitsAsync`, `TogglePin`, `_bankRowCat`), so the low-risk approach is a **separate "⇄ Transfer" action/modal** on the debit row (pick destination account+fund) rather than overloading the category `<select>` — keeps the expense+mapping flow untouched. **Can't be verified in local dev (bank is prod-only)** — build + careful review, then test on prod.
