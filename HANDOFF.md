...
Last updated: 2026-07-29 (Session 69 — **native Android catch-up**; web still live on `finapp-00254-gvk`). Read this + [README.md](README.md) + [TRANSFER.md](TRANSFER.md) + [docs/MOBILE.md](docs/MOBILE.md) + [docs/DOMAIN-REMOVAL.md](docs/DOMAIN-REMOVAL.md) + [android/README.md](android/README.md) + recent `git log` to catch up.
...
- **FAB overlaps scrolling content** — the centre FAB floats over content; a bottom-padding stopgap (96dp) helps but the **proper fix is a `BottomAppBar` cradle** (FAB docked, tabs split 2-and-2). Recommended next.
- **Not yet ported** (from the polish menu, user picked only the Spending view): **Health score gauge** (web = semicircular arc; Android = flat meter), **Goals conic rings** (web ProgressRing; Android = flat bars), expandable Goals rows.
