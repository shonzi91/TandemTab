...
Last updated: 2026-07-30 (Session 72 — **huge native-Android design pass: category line-icons, compact header + period switching, dark hero glow/gradient, Spending rework, add-sheet declutter, inline expense edit+delete, edit-last income, launcher icon, +more**; emulator-verified on a throwaway account, committed to `main`). Read this + [README.md](README.md) + [TRANSFER.md](TRANSFER.md) + [docs/MOBILE.md](docs/MOBILE.md) + [docs/DOMAIN-REMOVAL.md](docs/DOMAIN-REMOVAL.md) + [android/README.md](android/README.md) + recent `git log` to catch up.
...
### ⚠️ Carry-over / next
- **Bank sync (task open):** Android has **no** bank feature. Web endpoints exist (`/bank/status|institutions|link|sync|pending|accounts`, verified-email + allowlist gated). Realistic slice for an already-web-linked account: a "Sync now" (`POST /bank/sync`) + a pending-transactions import view (`GET /bank/pending`). Full link flow = an OAuth redirect (like Google sign-in deep-link) + consent + institution picker — large. **Decide scope with the user.**
- **Goals expandable sections (task open):** port the web bucket features + fancier add/spend icons.
- **Write paths** (budgets, categories, deposit edit, expense delete) are wired + emulator-verified on the **test** account. The launcher icon isn't eyeballed in the drawer yet.
