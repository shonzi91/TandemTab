...
Last updated: 2026-08-05 (Session 86 — **follow-up round, DEPLOYED as `finapp-00274-9mt`.** Fixed the alert chips (the scoped CSS could never apply — my prior "verified" was a self-inflicted false positive), **capped the free beta at 30 seats** with test-account exclusion by construction, added an **admin plan-override** so the upgrade journey can be rehearsed while wiring Stripe, added the **review-moderation door the approval gate was missing** (which is why the landing carousel was empty), tightened the feedback rate limit 30/min → 5/hour, and reworked the landing lower section. 293 server tests green. Prior context below is Session 85.)
