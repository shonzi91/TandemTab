...
Last updated: 2026-07-31 (Session 76 — **web-only follow-up: moved the Home "At this rate → Show the math" out of an inline expand and into a modal (it was shifting the whole page); confirmed the notification bell de-dup already meets "show only if hidden, don't duplicate the bell items". Deployed `finapp-00261-s6p`; committed `f111482` + pushed**). Read this + [README.md](README.md) + [TRANSFER.md](TRANSFER.md) + recent `git log` to catch up.
