...
Last updated: 2026-07-03 (Session 16). Read this + [README.md](README.md) + [TRANSFER.md](TRANSFER.md) + recent `git log` to catch up.
...
- **Debt & goals epic** (2026-07-05): budgeting tracks assets but not liabilities. Deploying a savings bucket to a goal (e.g. loan prepayment) shouldn't be an expense (it's not consumption) nor dent the savings rate. **Step 1 shipped:** `Period.DisburseSaving` (a savings bucket → external-transfer-out, not an expense; the paired drawdown is flagged `SourceExternalTransferId`/`IsDisbursement` so it drains the earmark but is added back in `SavingsReportService.PeriodSavingsRate`). UI: 🎯 "Apply to a goal" on a bucket. **Remaining:** (2) a **Loan/Debt entity** (balance, rate, min-payment) linkable to buckets/contributions — the keystone; (3) **payoff projection** ("at €X/mo you clear €100k in ~Y years"); (4) **prepay strategies** (avalanche/snowball, extra-payment "what-if"); (5) **achievements/badges** for savings pace & habits; plus a **net-worth** metric (assets − liabilities) alongside the savings rate.
