...
## Session 44 (2026-07-23) — Mobile Phase 1, the WRITES begin: the mutation-API foundation (spine + concurrency hardening + bootstrap + expenses + deposits). COMMITTED, PUSHED & DEPLOYED — live on `finapp-00216-hdq`.
One commit `c9b3e1c` → image `finapp:c9b3e1c` (digest `sha256:dc2bdab1…`, Cloud Build 3m58s) → **`finapp-00216-hdq`** (100%). Post-deploy: run URL + tandemtab.com both **200**, **5 `secretKeyRef`**, and the only WARNING+ on the revision were **2 benign `404 /sw.js`** (a browser service-worker probe — pre-existing, unrelated to this backend-only change). Tests **388 green** (224 domain + 44 persistence + **120 server**, +22 this session). This is the **write half of the Option-A migration** starting — see [docs/MOBILE.md](docs/MOBILE.md). **Nothing is wired into the web client** (reads-first discipline holds for writes too), so this deploy changes **no existing web behaviour** — it only adds new, web-unused endpoints. The one shared-code change is behaviour-preserving (below).
