...
Last updated: 2026-08-03 (Session 81 — **Debt R1 "informative debt" shipped full-stack + F3 "left to spend today" + UX #11 accessibility. `FinApp.Forecasting` + `FinApp.Domain` + `FinApp.Contracts` + `FinApp.Server` + `FinApp.Persistence` + `FinApp.Shared.UI` all touched. 551 tests green (262 domain + 244 server + 45 persistence). Browser-verified on seeded debts. NOT deployed, NOT committed until this entry. R2 (installment split) still not started.**). Read this + [README.md](README.md) + recent `git log` to catch up.
...
- **⚠️ NOT deployed, NOT committed at time of writing** (staged). **R2 (installment split + hybrid balance) still not started** — the medium-risk phase. Remaining R1 UI polish all verified. To deploy: the usual 3-step (`builds submit` → `run deploy --image` → `update-traffic --to-latest`) + served-bytes proof.
