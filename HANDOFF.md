...
**Backlog is now fully cleared (P0–P3).** Un-backlogged candidates for next: **push notifications / PWA** (the one thing #10 explicitly deferred — needs a service worker), **debt-lifecycle Phase 3** (a payment lowering a debt's remaining-owed; debt-paid/goal-reached archive prompts), **multiple synced funds** (one per linked bank account), and a **`RecurringItem` server/persistence test** if the snapshot round-trip ever grows teeth. HEAD at handoff: `2a172e3`.
