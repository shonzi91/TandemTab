...
Last updated: 2026-07-31 (Session 77 — **web-only: over-budget Home alert now carries a ↻ to rotate through multiple over-budget categories (the rotate control only existed in the bell, so extras were unreachable from Home); removed the "N reminders in notifications" Home link (the bell keeps those). ⚠️ COMMITTED + PUSHED but NOT DEPLOYED — user asked to hold; live remains `finapp-00261-s6p`. Commit `71c22b7`**). Read this + [README.md](README.md) + [TRANSFER.md](TRANSFER.md) + recent `git log` to catch up.
