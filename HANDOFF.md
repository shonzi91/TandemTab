...
Last updated: 2026-07-12 (Session 24). Read this + [README.md](README.md) + [TRANSFER.md](TRANSFER.md) + recent `git log` to catch up.
...
- **PlannedExpense bucket kind — NEW, shipped.** Added `SavingKind.PlannedExpense` (4th kind; body data, no migration — same pattern as Debt/Investment). It's a **goal-based bucket** (its goal = the target cost) that saves like a common bucket but reads as a planned expense — the honest label for money set aside for a known upcoming cost inside a bank-synced account. `SavingCategory.MarkPlannedExpense()` (sets kind, clears debt/investment fields, keeps the goal) + `Account.MarkSavingPlannedExpense`; serializer restore + EF `Ignore(IsPlannedExpense)`; threaded through `BudgetingState.Add/SaveSavingBucket` (`isPlannedExpense` flag). UI: 4th "🧾 Planned expense" chip in the bucket modal (reuses the goal field, relabelled "Target cost"), a new **🧾 Planned expenses** section on the Debt/Savings tab (mirrors Investments), and kind-aware labels ("🎉 Fully funded!", "🎯 Pay for it"). **Tests:** +2 domain (`SavingsTests`: kind/goal kept, mutual exclusivity) +1 serializer round-trip. **151 domain + 37 persistence + 82 server = 270 green; full solution builds.** Verification: build + unit tests only — **not browser-driven** (needs the server+web stack + a seeded account; fold into the Phase 0 pass).
