...
Last updated: 2026-08-01 (Session 79 — **seven period-lifecycle + money-consistency fixes from daily use (prompt-to-start-next-period, onboarding retires after period 1, remove-period crash, hero-vs-Breakdown Spent, Breakdown income incl. carryover, carryover incl. synced-fund closing, closed-period hero cards). DEPLOYED `finapp-00263-dzc`. Commit `9a84c74`. ⚠️ #6 changes the savings-rate denominator — see note.**). Read this + [README.md](README.md) + [TRANSFER.md](TRANSFER.md) + recent `git log` to catch up.
