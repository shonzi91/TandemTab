...
Last updated: 2026-08-04 (Session 82 — **Debt R2 "installment split + hybrid balance" shipped full-stack, plus its recurring-bill follow-up, plus BUG-1 (sign-out crash) fixed. `FinApp.Domain` + `FinApp.Contracts` + `FinApp.Server` + `FinApp.Persistence` + `FinApp.Shared.UI` touched. 588 tests green (287 domain + 253 server + 48 persistence). Browser-verified end-to-end on a throwaway. ⚠️ **NOT DEPLOYED** — the live revision is still S81's `finapp-00266-jbc`. Also still **unpushed**: S81's 4 commits + this session's.**). Read this + [README.md](README.md) + recent `git log` to catch up.
...
- **⚠️ NOT DEPLOYED and NOT PUSHED.** Live is still `finapp-00266-jbc` (S81). To ship: 3-step recipe (`builds submit` → `run deploy --image` → `update-traffic --to-latest`), then verify served bytes (`inst-split`/`inst-extra-row`/`inst-tag` in the scoped bundle) on both hosts + `secretKeyRef`=5. **No `app.css` change → no cache-bust.**
