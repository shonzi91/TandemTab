...
Last updated: 2026-07-26 (Session 55). Read this + [README.md](README.md) + [TRANSFER.md](TRANSFER.md) + [docs/MOBILE.md](docs/MOBILE.md) + [docs/DOMAIN-REMOVAL.md](docs/DOMAIN-REMOVAL.md) + [android/README.md](android/README.md) + recent `git log` to catch up.
...
- **Token is in-memory** — reopening the app requires signing in again. **Persistent token store (DataStore) + refresh (`POST /auth/refresh`) is the clear next slice** (I asked; user hasn't picked yet), then port Goals (`Savings`) / Wallets (`Account`) tabs. Home + Spending are built.
...
- **Trivial cleanup:** `Icons.Rounded.ReceiptLong` → `Icons.AutoMirrored.Rounded.ReceiptLong` (deprecation warning, Spending nav icon).
