...
Last updated: 2026-07-09 (Session 22). Read this + [README.md](README.md) + [TRANSFER.md](TRANSFER.md) + recent `git log` to catch up.
...
**Spending tab: dropped the calendar view (`0a2e82c`, finapp-00122-zz4).** Removed the month-grid calendar from **both** the expenses list and the "From your bank" review list. The view toggle is now **☰ List** (grouped by date) / **📆 By day** (`ShowDayView` → single day with prev/next date nav, defaults to today). Deleted the dead `_calendar`/`_bankCal` fields, `CalendarDays`, `ShowCalendarView`, `OpenDayFromCalendar`, `OpenAddExpenseOn`. NB the `cal-*` CSS classes in `Dashboard.razor.css` are now unused (left in place — harmless dead CSS; remove if you touch that file). Verified live: toggle renders List/By-day, no calendar grid, no console errors.
