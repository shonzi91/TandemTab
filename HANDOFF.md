...
printed `CN=Stoyan Stoyanov, OU=TandemTab Company, O=TandemTab Company, L=Sofia, ST=Sofia, C=BG` instead of
`CN=Android Debug`, so the debug-key fallback is no longer in play and CI artifacts are updatable in place.
