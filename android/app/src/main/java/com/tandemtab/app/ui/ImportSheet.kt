...
private fun ImportReviewRow(
    ...
    Row(Modifier.fillMaxWidth().padding(vertical = 6.dp), verticalAlignment = Alignment.CenterVertically) {
        ...
        Text(
            // Built by hand rather than by `moneyFormatter` because the sign carries meaning here, so the mask
            // has to be applied by hand too. These are figures read out of a file and shown as data — the same
            // class of thing as an expense row, which masks everywhere else. The `currencySymbol` prefixes on
            // the amount FIELDS in the other sheets are deliberately not masked: you cannot hide a number the
            // user is typing at that moment, and the web draws the same line.
            if (Privacy.masked) Privacy.MASKED
            else (if (isExpense) "−" else "+") + currencySymbol(currency) + trimAmount(kotlin.math.abs(row.tx.amount)),
            fontSize = 14.sp,
