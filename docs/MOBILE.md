...
> | ~~**The Breakdown donut**~~ ✅ **SHIPPED (S115–S117)** | This row said *"`GET /accounts/{id}/breakdown` does not exist — no server read stands behind it"*, and it was right when written. The endpoint was written in S115, the ring shipped in the same session, and S117 gave it a visible door on Home. ★ Its diagnosis held all the way through: it was a **server-read row**, and every attempt to size it as Kotlin was sizing the wrong half. | — | ⚠️ Struck through rather than deleted, because the *reasoning* is the reusable part — and `GET /trends` and `GET /savings/plan` (S118) were both found the same way. |
